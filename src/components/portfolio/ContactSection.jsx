      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        >
          ...
          <p className="text-xl text-charcoal/70 max-w-3xl mx-auto leading-relaxed">
            Ready to discuss your next venture, partnership, or development project? Let's explore how we can work together.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
            <Card className="bg-pure-white shadow-lg border-0 rounded-2xl">
              <CardHeader className="p-8">
                <CardTitle className="text-2xl font-light text-charcoal">Send a Message</CardTitle>
              </CardHeader>
              <CardContent className="p-8 pt-0">
                <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
                  <div>
                    <label className="block text-sm font-medium text-charcoal/70 mb-2">What would you like to discuss?</label>
                    <Select name="inquiry_type" value={formData.inquiry_type} onValueChange={(value) =>
                      setFormData((prev) => ({
                        ...prev,
                        inquiry_type: value,
                        // if subject is empty, prefill from dropdown
                        subject: prev.subject?.trim() ? prev.subject : SUBJECT_BY_TYPE[value] || prev.subject,
                      }))
                    }>
                    ...
                    <p className="text-sm text-charcoal/60 mt-2">
                      {inquiryTypes.find((t) => t.value === formData.inquiry_type)?.description}
                    </p>
                  </div>
          ...
          {/* Right: contact details + response times */}
          <motion.div
          ...
          >
            <Card className="bg-pure-white shadow-lg border-0 rounded-2xl">
              <CardHeader className="p-8">
                <CardTitle className="text-2xl font-light text-charcoal">Get in Touch</CardTitle>
              </CardHeader>
              <CardContent className="p-8 pt-0 space-y-6">
                <div className="flex items-start space-x-4">
                  <Mail className="w-5 h-5 text-deep-emerald mt-1 flex-shrink-0" />
                  <div>
                    <p className="font-medium text-charcoal">Email</p>
                    <p className="text-charcoal/70">wiem.mimouni@voyagr.ca (Primary contact)</p>
                    <p className="text-charcoal/70">wiemmimouni74@gmail.com (Personal contact)</p>
                  </div>
                </div>
                <div className="flex items-start space-x-4">
                  <MessageCircle className="w-5 h-5 text-deep-emerald mt-1 flex-shrink-0" />
                  <div>
                    <p className="font-medium text-charcoal">WhatsApp</p>
                    <p className="text-charcoal/70">+1 437-830-1154 (Direct messaging)</p>
                  </div>
                </div>
                <div className="flex items-start space-x-4">
                  <MapPin className="w-5 h-5 text-deep-emerald mt-1 flex-shrink-0" />
                  <div>
                    <p className="font-medium text-charcoal">Location</p>
                    <p className="text-charcoal/70">Canada • Tunisia • Europe • US</p>
                    <p className="text-sm text-charcoal/60">(Remote-first with global presence)</p>
                  </div>
                </div>
                <div className="flex flex-col sm:flex-row gap-3 pt-4">
                  <Button className="btn-primary flex-1" onClick={() => window.open('https://wa.me/14378301154', '_blank')}>
                  ...
                  >
                    <Calendar className="w-4 h-4 mr-2 text-deep-emerald group-hover:text-white transition-colors" />
                    Schedule a Call
                  </Button>
                </div>
              </CardContent>
            ...
            <Card className="bg-pure-white shadow-lg border-0 rounded-2xl">
              <CardHeader className="p-8">
                <CardTitle className="text-2xl font-light text-charcoal flex items-center">
                  <Clock className="w-6 h-6 mr-3 text-deep-emerald" />
                  Response Time
                </CardTitle>
              </CardHeader>
              <CardContent className="p-8 pt-0">
                <div className="space-y-4">
                  {responseTimes.map((item, index) => (
                    <motion.div
                      ...
                      viewport={{ once: true }}
                      className="flex items-center justify-between p-3 bg-light-neutral/50 rounded-lg"
                    >
